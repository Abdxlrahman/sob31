...
def compare_numbers(number,user_guess):
    # AR completed the code needed here
    cows = 0
    bulls = 0
    ...
    for i in range(len(nummber)):
        if user_guess[i] == number [i]:
            ...
            cows += 1
    return cows,bulls # AR fixed 'cowbull' to 'cows,bulls'

playing = True #gotta play the game
number = str(random.randint(1000,9999)) #random 4 digit number # AR fixed 0 to 1000 to get 4 digits number
guesses = 0
print(number) # AR added parentheses() for print statement
